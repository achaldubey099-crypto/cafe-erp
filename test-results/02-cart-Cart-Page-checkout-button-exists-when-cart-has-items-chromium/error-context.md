...
- Name: 02-cart.spec.ts >> Cart Page >> checkout button exists when cart has items
- Location: tests/02-cart.spec.ts:50:7
...
Locator: locator('button').filter({ hasText: 'Checkout' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: 'Checkout' })
